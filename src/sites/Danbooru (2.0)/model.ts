            ...
            search: {
                ...
                url: (query: ISearchQuery, opts: IUrlOptions, previous: IPreviousSearch | undefined): string | IError => {
                    try {
                        ...
                        return "/posts.json?limit=" + opts.limit + "&page=" + pagePart + "&tags=" + encodeURIComponent(Grabber.normalizeOrderTags(query.search, "danbooru"));
                    } catch (e: any) {
        ...
        xml: {
            ...
            search: {
                ...
                url: (query: ISearchQuery, opts: IUrlOptions, previous: IPreviousSearch | undefined): string | IError => {
                    try {
                        const pagePart = Grabber.pageUrl(query.page, previous, 1000, "{page}", "a{max}", "b{min}");
                        return "/posts.xml?limit=" + opts.limit + "&page=" + pagePart + "&tags=" + encodeURIComponent(Grabber.normalizeOrderTags(query.search, "danbooru"));
                    } catch (e: any) {
                        return { error: e.message };
                    }
                },
            ...
            check: {
                ...
                parse: (src: string): boolean => {
                    return src.indexOf("Running Danbooru v2") !== -1
                        ...
                        || /https:\/\/github\.com\/danbooru\/danbooru(?:[/"'?#]|$)/.test(src)
                        || src.indexOf("ATFBooru") !== -1
                        || src.indexOf("All The Fallen") !== -1
                        || /https:\/\/github\.com\/cosbooru\/cosbooru(?:[/"'?#]|$)/.test(src)
                        || /https:\/\/github\.com\/Iratu\/atfbooru(?:[/"'?#]|$)/.test(src);
                },
            },
        },
    },
